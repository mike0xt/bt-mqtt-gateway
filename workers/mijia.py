# For custom firmware pvvx v3.1, atc1441 format
# lywsd03mmc also changed [breaking] to include bat % and V
# Added RSSI
##
...
monitoredAttrs = ["temperature", "humidity", "battery_level", "battery_voltage", "rssi"]
_LOGGER = logger.get(__name__)

class MijiaWorker(BaseWorker):
    """
    ...
    def config_device(self, name, mac):
        ...
        for attr in monitoredAttrs:
            ...
            if attr == "humidity":
                ...
                payload.update({"device_class": "signal_strength", "unit_of_measurement": "dBm"})
...
## Added rssi
                    ...
                    _LOGGER.debug(f"!!!!Signal strength: {device.rssi}")
###
                    for (adtype, desc, value) in res.getScanData():
                        _LOGGER.debug(f"!!!adtype: {adtype}, desc: {desc}, value: {value}")
                        if ("1a18" in value):
